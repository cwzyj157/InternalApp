...
namespace InternalApp.Controller.PointOfSale.Services
{
	public class OrderService
	{
		...
		[Action]
		public object Search(DateTime dt, string type)
		{
			if( type == "0" ) {
				...
				// 返回一个任意对象也是可以的
				//return new UcResult("~/Controls/PointOfSale/OrderList1.ascx", new FoodsBLL().GetOrderInfos(dt));

				PageOrderInfoModel data = new PageOrderInfoModel();
				data.OrderInfos = new FoodsBLL().GetOrderInfos(dt);
				return new UcResult("~/Controls/PointOfSale/OrderList1.ascx", data);
			}
		}
